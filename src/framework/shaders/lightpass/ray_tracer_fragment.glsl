...
uniform vec2 in_World; // Size of the world/collision texture we're tracing rays against. (1/size)
...
void main() {
	...
	for(float v = step(xyRay.x,xyRay.y), d = 0.; d < MAXRADIUS * v; d++)
		...
		*/
		if (in_Light.z < d + in_Light.z * texture2D(Texture, (in_Light.xy + (xyRay = Delta * d)) * in_World).a) break;
