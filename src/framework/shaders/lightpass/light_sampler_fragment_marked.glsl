...
//in_World: size (w+h) of the texture collision texture (1/real_size)
...
void main() {
	...
	//current pixel's coordinate relative to the destination texture
	...
	//float RayIndex = (RayCount * fract(atan(-Delta.y, Delta.x)/TAU));
	//float RayIndex = (RayCount * fract(atan(-14.0, 80.0)/TAU));
	//float RayIndex = 64.;
	...
	float RayVisible = sign(RayLength - Distance) * (1. - texture2D(Texture, (in_Light.xy + Delta) * in_World).a);
	...
	//gl_FragColor = texture2D(Texture, in_Coord);
	//gl_FragColor = vec4(vec2(abs(Delta) * in_World), 0., 1.);
