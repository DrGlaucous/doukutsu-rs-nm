...
impl<T: std::cmp::PartialEq + std::default::Default + Clone> Menu<T> {
    ...
    pub fn put_board(state: &mut SharedGameState, ctx: &mut Context, x: f32, y: f32, width: u16, height: u16) -> GameResult {
        ...
        let self_x = x;//self.x as f32;
        let self_y = y;//computed_y;
        let self_width = width;//self.width;
        let self_height = height;//self.height;

        let mut x = self_x as f32;
        let mut y = self_y;
        let mut width = self_width;
        let mut height = self_height;

        rect = state.constants.title.menu_left_top;
        batch.add_rect(x as f32 - rect.width() as f32, y - rect.height() as f32, &rect);
        rect = state.constants.title.menu_right_top;
        batch.add_rect(x as f32 + width as f32, y - rect.height() as f32, &rect);
        rect = state.constants.title.menu_left_bottom;
        batch.add_rect(x as f32 - rect.width() as f32, y + height as f32, &rect);
        rect = state.constants.title.menu_right_bottom;
        batch.add_rect(x as f32 + width as f32, y + height as f32, &rect);
        ...
        while width > 0 {
            ...
            batch.add_rect(x, y + self_height as f32, &rect2);
            x += rect.width() as f32;
        }
        ...
        while height > 0 {
            ...
            batch.add_rect(x + self_width as f32, y, &rect2);
